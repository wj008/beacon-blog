...
class ZeroColumn extends ZeroController
{

    //为ZeroController所需的配置信息
    protected function zeroLoad(){ 
        return [
            'actionForm' => 'app\\admin\\zero\\form\\ZeroColumnForm',
            'searchForm' => 'app\\admin\\zero\\form\\ZeroColumnSearch',
            'tbname' => '@pf_column',
            'template' => 'ZeroColumn.tpl',
            'templateHack' => 'ZeroColumn.hack.tpl',
        ];
    }
    ...
    protected function zeroWhere(SqlSelectInterface $selector){ 
          //从搜索表单获取数据
        $search = Form::instance('app\\admin\\zero\\form\\ZeroColumnSearch');
    ...
    protected function zeroSelector(){ 
        $selector = new SqlSelector('@pf_column');

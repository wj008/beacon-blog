...
class ZeroLink extends ZeroController
{

    //为ZeroController所需的配置信息
    protected function zeroLoad(){ 
        return [
            'actionForm' => 'app\\admin\\zero\\form\\ZeroLinkForm',
            'searchForm' => 'app\\admin\\zero\\form\\ZeroLinkSearch',
            'tbname' => '@pf_link',
            'template' => 'ZeroLink.tpl',
            'templateHack' => 'ZeroLink.hack.tpl',
        ];
    }
    ...
    protected function zeroWhere(SqlSelectInterface $selector){ 
          //从搜索表单获取数据
        $search = Form::instance('app\\admin\\zero\\form\\ZeroLinkSearch');
    ...
    protected function zeroSelector(){ 
        $selector = new SqlSelector('@pf_link');
